...
function getMailFiles(res,mule){
	fs.readdir("/var/spool/mail/", function(err, items) {
       ...
       // var filesize = getFilesizeInBytes("/var/spool/mail/"+items[i]);
        	if ((items[i].includes(config.overalldomain))&&(!items[i].includes("mail"))){
       		 	resfiles[x++]=items[i];
    		}
		}
...
function filterOnMule(mule, mailFiles, res){
    ...
    //Filter when subdomains are non zero
  		for(var i = 0; i < mule.subdomains.length;i++){
    	    ...
    	    if (mailFiles.indexOf(mule.subdomains[i].subdomain+".mule.global") > -1) {
   				 resMailFiles[x++]=mailFiles[mailFiles.indexOf(mule.subdomains[i].subdomain+".mule.global")];
...
var server = app.listen(SERVER_PORT, function() {
    console.log("Emule server started. Listening on all interfaces on port " +
                server.address().port);
